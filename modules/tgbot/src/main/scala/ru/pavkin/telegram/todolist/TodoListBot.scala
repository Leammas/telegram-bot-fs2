...
class TodoListBot[F[_]](api: StreamingBotAPI[F],
  ...
  private def clearTodoList(chatId: ChatId): F[Unit] =
    for {
      _ <- storage.clearList(chatId)
      _ <- logger.info(s"todo list cleared for chat $chatId") *> api
        .sendMessage(chatId, "Your todo-list was cleared!")
    } yield ()

  private def showTodoList(chatId: ChatId): F[Unit] =
    for {
      items <- storage.getItems(chatId)
      _ <- logger.info(s"todo list queried for chat $chatId") *> api
        .sendMessage(
          chatId,
  ...
  private def addItem(chatId: ChatId, item: Item): F[Unit] =
    for {
